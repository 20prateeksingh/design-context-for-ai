...
- The headline capability worth naming: **⧉ Copy for Figma** — on any page (Home atlas panel, Map panel, page doc, or a captured state) one click copies that snapshot to the clipboard and it pastes into Figma as **editable auto-layout layers**. Don't name a paste shortcut — the designer may be on Windows, where `⌘V` is wrong; the dashboard's own copy is platform-aware and this file can't be. No plugin, no Dev Mode, no paid seat (prompt canon **A.10**).
