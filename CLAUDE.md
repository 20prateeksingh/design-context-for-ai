...
Start at `design-context/registry.json` — every page keyed by slug with route, label, files, link graph (`linksTo`/`linkedFrom`), template info, and a labeled description. Additive dashboard-v2 fields ride alongside (never mutate existing ones): per-page `inboundCount` + `clickDepth` (measured clicks from the home page), and top-level `identity` (name + observed meta description), `readiness` (composite context-readiness score + breakdown), and `events` (the journal feed — capture, tokens, describe, states, wireframe rounds — derived from embedded timestamps). Humans start at `INDEX.md` (same content, readable) or the dashboard (Home atlas + ledger · Map · Design language · Use it · Journal). Each page folder: `page.md` (digest — facts + a "What this page is" section), `screenshot.png`, `page.html` (the editable baseline), `content.md` (verbatim copy), `computed-tokens.json`, `meta.json`.
